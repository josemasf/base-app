...
const layout = new Layout({
  ...
  const vueCategories = layout.client.register({
    name: "vueCategories", // required    
    uri: `${process.env.VUECATEGORIES_URL}manifest.json`
  });
  ...
  module.exports = {
    view: async function (req, res) {
        ...
        const [header, login, footer, categories] = await Promise.all([     
            vuehead.fetch(incoming),
            vueLogin.fetch(incoming),
            vueFooter.fetch(incoming),
            vueCategories.fetch(incoming),
        ]);

        //binding the podlet data to the layout
        incoming.podlets = [ header, login, footer, categories];
        ...
        <div class="columns">          
          <div class="column">
            <section class="section">${categories.content}</section>
          </div>
        </div>
        <div class="columns">
          <div class="column">
